...
def drawAll():
    ...
    if layout.findWidget('draw lines').checkButtonValue() :
        if True:
            # draw lines
            goal=m.matrixn(36,3)
            goal.setAllValue(0)
    ...
    if layout.findWidget('draw colored lines').checkButtonValue() :
        if True:
            ...
            RE.drawBillboard( goal.matView(), 'goal3c2', 'use_vertexcolor_only', thickness, 'ColorBillboardLineList' )
        if True:
            # point list uses a colormap
            
            points=m.matrixn(36, 6) #-- each row contains (R, G, B, x, y, z)
            for i in range(1,36+1) :
                points.row(i-1).setVec3(3, m.vector3(0,140, i*10))
                R=m.map(i, 1,36, 1,0)
                # colormap.jpg (B channel represents point size)
                G=0
                B=10 # thickness
                color=m.vector3(R, G, B)
    ...
    if layout.findWidget('draw meshes').checkButtonValue() :
        if True:
            # draw axes
            lref1=m.transf()
        ...
        if True:
            ...
            _, node=mesh.drawMesh("checkboard/crowdEditing", "nodeName")
            node.translate(0,55,0)

        if True:
            # draw custom mesh
            mesh2=m.Geometry()
            mesh2.initPlane(40, 40) # sizeX, sizeY
            meshToEntity2, node2=mesh2.drawMesh("red", "nodeName2")
            ...
            #meshToEntity2.updatePositionsAndNormals()
            meshToEntity2.updatePositions()
...
def eraseAll():
    RE.eraseAllDrawn() # all objects drawn using RE.draw or RE.namedDraw
    RE.removeEntityByName("nodeName")
...
layout.create("Check_Button", 'draw lines', 'draw lines (much faster)')
layout.findWidget('draw lines').checkButtonValue(True)
layout.create("Check_Button", 'draw colored lines', 'draw colored lines')
layout.findWidget('draw colored lines').checkButtonValue(True)
layout.create("Check_Button", 'draw meshes', 'draw meshes')
layout.findWidget('draw meshes').checkButtonValue(True)
